...
describe.skip('Send component', () => {
  ...
  beforeEach(inject((_$compile_, _$rootScope_, _moose_, _Account_, _AccountApi_) => {
    $compile = _$compile_;
  ...
  beforeEach(() => {
    ...
    account.set({
      ...
      balance: moose.from(10535.77379498),
    });
  ...
  describe('create transaction', () => {
    ...
    it('should allow to create a transaction', () => {
      const RECIPIENT_ADDRESS = '5932438298200837883M';
      const AMOUNT = '10';
    ...
    it('should allow to send all funds', () => {
      const RECIPIENT_ADDRESS = '5932438298200837883M';
      const AMOUNT = moose.normalize(account.get().balance - 10000000);
      ...
      expect(spy).to.have.been.calledWith({ text: `${AMOUNT} MOOSE was successfully transferred to ${RECIPIENT_ADDRESS}` });
      mock.verify();
    });
  });
});
...
describe('Send component controller', () => {
  ...
  describe('send()', () => {
    it('calls accountApi.transactions.create and success.dialog on success', () => {
      ...
      $scope.$apply();
      expect(spy).to.have.been.calledWith({
        text: `Your transaction of ${controller.amount.value} MOOSE to ${controller.recipient.value} was accepted and will be processed in a few seconds.`,
      });
    });
