  ...
  controller: class send {
    constructor($scope, moose, dialog, $mdDialog, $q, $rootScope, Account, AccountApi) {
      ...
      if ($scope.$ctrl.sendAmount) {
        this.amount.value = parseFloat(moose.normalize($scope.$ctrl.sendAmount), 10);
      }

      this.$scope.$watch('$ctrl.amount.value', () => {
        if (moose.from(this.amount.value) !== this.amount.raw) {
          this.amount.raw = moose.from(this.amount.value) || 0;
        }
      });

      this.$scope.$watch('$ctrl.account.balance', () => {
        this.amount.max = moose.normalize(this.account.get().balance - 10000000);
      });
    ...
    send() {
      ...
      this.accountApi.transactions.create(
        ...
        return this.dialog.successAlert({
          text: `Your transaction of ${this.amount.value} MOOSE to ${this.recipient.value} was accepted and will be processed in a few seconds.`,
        }).then(() => {
          this.reset();
        });
      }).catch((res) => {
